...
2. [What is an Agent Skill (and How to Build Your First One)](#2-what-is-an-agent-skill-and-how-to-build-your-first-one) — 6
   - [Skill Anatomy & Progressive Disclosure](#skill-anatomy--progressive-disclosure) — 7
   - [Path A: Translating what you already know](#path-a-translating-what-you-already-know) — 8
   - [Path B: Crystallizing what the agent just did](#path-b-crystallizing-what-the-agent-just-did) — 9
     - [Trying it out & How to install Skills today](#trying-it-out--how-to-install-skills-today) — 10
     - [Wait but how does a Skill differ from MCP and AGENTS.md?](#wait-but-how-does-a-skill-differ-from-mcp-and-agentsmd) — 11
3. [Why did Agent Skills become so popular, so fast?](#3-why-did-agent-skills-become-so-popular-so-fast) — 12
   - [Skills for chatbots, for coding agents, or for multi-agent enterprise use cases?](#skills-for-chatbots-for-coding-agents-or-for-multi-agent-enterprise-use-cases) — 13
4. [Evaluating Skills](#4-evaluating-skills) — 15
   - [The Evaluation Toolkit](#the-evaluation-toolkit) — 17
     - [The trigger is the first gate](#the-trigger-is-the-first-gate) — 18
     - [Output quality and tool trajectory](#output-quality-and-tool-trajectory) — 19
       - [System vs. Skill: The Evaluation Illusion](#system-vs-skill-the-evaluation-illusion) — 21
     - [Token budget: isolation is a trap](#token-budget-isolation-is-a-trap) — 22
       - [What "eval coverage" means](#what-eval-coverage-means) — 24
5. [From Prototype to Production](#5-from-prototype-to-production) — 25
   - [What's actually inside an agent runtime](#whats-actually-inside-an-agent-runtime) — 27
   - [Why skills are the unit of improvement](#why-skills-are-the-unit-of-improvement) — 28
     - [The failure mode that breaks demos: context overflow](#the-failure-mode-that-breaks-demos-context-overflow) — 29
     - [What this means for the token budget](#what-this-means-for-the-token-budget) — 31
6. [On Meta-Skills and Self-Improving Skills](#6-on-meta-skills-and-self-improving-skills) — 32
   - [Where this falls apart](#where-this-falls-apart) — 34
   - [Where this is going](#where-this-is-going) — 35
7. [Composing and Packaging Skills](#7-composing-and-packaging-skills) — 36
   - [Execution Routing: DAG Orchestration](#execution-routing-dag-orchestration) — 36
   - [Environment Packaging: Capability Profiles](#environment-packaging-capability-profiles) — 37
   - [Populating the Graph: The Canonical Skill Taxonomy](#populating-the-graph-the-canonical-skill-taxonomy) — 37
   - [Context Debt and Shifting Intelligence Left](#context-debt-and-shifting-intelligence-left) — 38
   - [Architectural Tradeoffs](#architectural-tradeoffs) — 38
   - [Actionable Best Practices](#actionable-best-practices) — 39
8. [How to Decide Among the Hundreds of Skills That Exist](#8-how-to-decide-among-the-hundreds-of-skills-that-exist) — 39
9. [Conclusion](#9-conclusion) — 41
- [Appendix A – The Practical Cheatsheet](#appendix-a--the-practical-cheatsheet) — 43
  - [The minimal SKILL.md](#the-minimal-skillmd) — 43
  - [The folder structure](#the-folder-structure) — 44
  - [Naming](#naming) — 44
  - [The description field](#the-description-field) — 45
  - [The five rules](#the-five-rules) — 45
  - [The quality principles](#the-quality-principles) — 46
  - [Do's and Don'ts](#dos-and-donts) — 46
  - [Skill smells (revise if you see these)](#skill-smells-revise-if-you-see-these) — 47
  - [Eval coverage checklist](#eval-coverage-checklist) — 48
  - [Deployment checklist](#deployment-checklist) — 48
  - [One-line mental model](#one-line-mental-model) — 49
  - [Where to start tomorrow](#where-to-start-tomorrow) — 49
- [Appendix B – Case Study: Domain Expertise as Code (Vertical Skills in Retail)](#appendix-b--case-study-domain-expertise-as-code-vertical-skills-in-retail) — 50
  - [Why retail is the canonical case for skills](#why-retail-is-the-canonical-case-for-skills) — 50
  - [What the architecture looks like](#what-the-architecture-looks-like) — 51
  - [An illustrative skills library](#an-illustrative-skills-library) — 52
  - [How does the same query route through the library](#how-does-the-same-query-route-through-the-library) — 53
  - [Ownership: who writes which skill](#ownership-who-writes-which-skill) — 54
  - [The read / draft / act ladder](#the-read--draft--act-ladder) — 55
  - [Why is this harder to compete with than a custom agent](#why-is-this-harder-to-compete-with-than-a-custom-agent) — 55
  - [Cold start: where to actually begin](#cold-start-where-to-actually-begin) — 56
- [Endnotes](#endnotes) — 60
...
**Description**. <u>This is your routing algorithm</u>. It is the only thing the model sees to decide whether to load the Agent Skill. State what it does, front-load trigger keywords, be pushy if the model under-triggers, and explicitly state what it is not for.
...
**Evaluation-Driven Development (EDD)**¹¹ inverts the workflow by writing three JSON evaluation cases (Input, Expected Tools, Expected Output) before drafting the SKILL.md. It forces a clear functional spec upfront. A minimal eval case looks like this:
...
**pass^k** measures consistent, rather than occasional, success by running the evaluation $k$ times and requiring success on every run. On tau-bench (Yao et al., 2024)¹⁴, GPT-4o scored 61% on pass^1 but dropped below 25% on pass^8, demonstrating that single-run success is a poor predictor of production reliability.
...
**A Skills-First Retail Architecture (Illustrative)** — *A generic large-format retailer running on a commodity agent runtime*

- **Customer surface** (web, mobile, in-store, voice – wherever the shopper is): retailer.com chat, Pro mobile app, In-store kiosk, Voice agent, SMS
- **Agent runtime** (commodity – ADK, Claude SDK, etc. – loads skills on demand): `project-guidance` ("how do I tile a shower?" — trades knowledge), `product-fit` ("is this faucet compatible with that valve?"), `materials-list` (project → grouped BOM for Pros), `store-locator` ("aisle 14, bay 7" — live inventory), `code-compliance` (local building code disclaimers), `review-summarize` (condense product reviews into pros/cons), `delivery-window` (last mile ETA and options), + ~50 more (owned by trades, merch, supply chain, legal teams)
- **Tools / data layer** (accessed via MCP & managed search integrations): Product catalog (2M+ SKUs, warehouse), Inventory (live store-level stock + locations), Customer profile (orders, returns, loyalty, location), Project KB (how-to guides, diagrams, manuals), Vector search (RAG over reviews, spec sheets, manuals)

*Skills carry the judgement (the brand's expertise). MCP carries the data plumbing. The model is interchangeable. Pattern is illustrative; several major retailers have publicly described systems matching this architecture.*
...
Google Cloud Blog. (2026, April). Level Up Your Agents: Announcing Google's Official Skills Repository. [https://cloud.google.com/blog/topics/developers-practitioners/level-up-your-agents-announcing-googles-official-skills-repository](https://cloud.google.com/blog/topics/developers-practitioners/level-up-your-agents-announcing-googles-official-skills-repository). Repository: [https://github.com/google/skills](https://github.com/google/skills). Installable via `npx skills install github.com/google/skills` for Antigravity, and any compliant coding agent.

---

## Endnotes

1. [https://codelabs.developers.google.com/getting-started-with-antigravity-skills?utm_campaign=CDR_0xf9030db1_default_b513044940&utm_medium=external&utm_source=blog#0](https://codelabs.developers.google.com/getting-started-with-antigravity-skills?utm_campaign=CDR_0xf9030db1_default_b513044940&utm_medium=external&utm_source=blog#0)
2. [https://github.com/anthropics/skills/tree/main/skills/skill-creator](https://github.com/anthropics/skills/tree/main/skills/skill-creator)
3. [https://hermes-agent.nousresearch.com/docs/user-guide/features/skills](https://hermes-agent.nousresearch.com/docs/user-guide/features/skills)
4. [https://github.com/Shubhamsaboo/awesome-llm-apps/tree/main/awesome_agent_skills/self-improving-agent-skills](https://github.com/Shubhamsaboo/awesome-llm-apps/tree/main/awesome_agent_skills/self-improving-agent-skills)
5. [https://adk.dev/skills/](https://adk.dev/skills/)
...
7. [https://arxiv.org/abs/2602.12670](https://arxiv.org/abs/2602.12670)
8. [https://vercel.com/blog/agents-md-outperforms-skills-in-our-agent-evals](https://vercel.com/blog/agents-md-outperforms-skills-in-our-agent-evals)
9. [https://latitude.so/blog/agent-first-comparison-guide-vs-braintrust](https://latitude.so/blog/agent-first-comparison-guide-vs-braintrust)
10. [https://adk.dev/evaluate/](https://adk.dev/evaluate/)
11. [https://arxiv.org/html/2411.13768v2](https://arxiv.org/html/2411.13768v2)
12. [https://arxiv.org/abs/2508.16260](https://arxiv.org/abs/2508.16260)
13. [https://research.trychroma.com/context-rot](https://research.trychroma.com/context-rot)
14. [https://arxiv.org/abs/2406.12045](https://arxiv.org/abs/2406.12045)
15. [https://arxiv.org/abs/2601.06112](https://arxiv.org/abs/2601.06112)
16. [https://arxiv.org/abs/2601.17087](https://arxiv.org/abs/2601.17087)
17. [https://google.github.io/agents-cli/](https://google.github.io/agents-cli/)
18. Liu, Zhao, Shang, and Shen (2026), reverse-engineering of Claude Code v2.1.88; companion site ccunpacked.dev
19. Liu et al., "Lost in the Middle" (TACL 2024). [https://arxiv.org/abs/2601.06112](https://arxiv.org/abs/2601.06112)
20. Chroma Research, "Context Rot" (2025). [https://arxiv.org/abs/2601.17087](https://arxiv.org/abs/2601.17087)
21. [https://developers.googleblog.com/en/developers-guide-to-building-adk-agents-with-skills/](https://developers.googleblog.com/en/developers-guide-to-building-adk-agents-with-skills/)
22. [https://github.com/anthropics/skills](https://github.com/anthropics/skills)
23. [https://github.com/anthropics/skills/blob/main/skills/skill-creator/SKILL.md](https://github.com/anthropics/skills/blob/main/skills/skill-creator/SKILL.md)
24. [https://github.com/Shubhamsaboo/awesome-llm-apps/tree/main/awesome_agent_skills/self-improving-agent-skills](https://github.com/Shubhamsaboo/awesome-llm-apps/tree/main/awesome_agent_skills/self-improving-agent-skills)
25. [https://github.com/karpathy/autoresearch](https://github.com/karpathy/autoresearch)
26. [https://arxiv.org/abs/2305.16291](https://arxiv.org/abs/2305.16291)
27. [https://github.com/philschmid/self-learning-skill](https://github.com/philschmid/self-learning-skill)
28. Google. (2026). Agents CLI: Unified CLI for the full ADK agent development lifecycle. [https://google.github.io/agents-cli/](https://google.github.io/agents-cli/). Source repository: [https://github.com/google/agents-cli](https://github.com/google/agents-cli). The framework ships seven skills covering scaffold, build, eval, deploy, publish, and observability for agents built on Google Cloud, designed to plug into any coding agent that supports the Agent Skills standard.
...
30. Google Developers Blog. (2026, March). Closing the knowledge gap with agent skills. [https://developers.googleblog.com/closing-the-knowledge-gap-with-agent-skills/](https://developers.googleblog.com/closing-the-knowledge-gap-with-agent-skills/). Reports the Gemini API developer skill improving Gemini 3.1 Pro from 28.2% to 96.6% on SDK code generation across 117 prompts. Source skill: [https://github.com/google-gemini/gemini-skills](https://github.com/google-gemini/gemini-skills).
31. Anthropic. (2025-2026). anthropics/skills — Public repository for Agent Skills. [https://github.com/anthropics/skills](https://github.com/anthropics/skills). Contains Anthropic's reference implementation of the Skills standard plus example skills for documents (docx, pdf, xlsx, pptx), MCP server building, and skill creation itself.
32. Stripe. (2026). stripe/ai — One-stop shop for building AI-powered products and businesses with Stripe. [https://github.com/stripe/ai](https://github.com/stripe/ai). Includes the stripe-best-practices skill (skills/stripe-best-practices/SKILL.md) for API selection, Connect setup, billing, and security; published and maintained by Stripe.
33. Microsoft. (2026). microsoft/skills — Skills, MCP servers, Custom Agents, Agents.md for SDKs to ground Coding Agents. [https://github.com/microsoft/skills](https://github.com/microsoft/skills). Microsoft's official skill collection for grounding coding agents in Azure SDKs and Microsoft Foundry.
34. VoltAgent. (2026). awesome-agent-skills — A curated collection of 1000+ agent skills from official dev teams and the community. [https://github.com/VoltAgent/awesome-agent-skills](https://github.com/VoltAgent/awesome-agent-skills). Features official skills from Anthropic, Google Labs, Vercel, Stripe, Cloudflare, Netlify, Trail of Bits, Sentry, Expo, Hugging Face, Figma, and others, alongside community-built skills. Compatible with Claude Code, Codex, Antigravity, Cursor, GitHub Copilot, OpenCode, and Windsurf.
35. Skills Marketplace. (2026). [https://skillsmp.com](https://skillsmp.com). An independent community-aggregated marketplace of 1.2M+ skills sourced from public GitHub repositories, with semantic search, occupation filtering, and minimum-quality indicators (2-star threshold). Marketplace operator explicitly recommends inspecting community skills before installation.
36. Osmani, A. (2026). addyosmani/agent-skills — Production-grade engineering skills for AI coding agents. [https://github.com/addyosmani/agent-skills](https://github.com/addyosmani/agent-skills).
